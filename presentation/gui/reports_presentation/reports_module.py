...
class ReportModule(ttk.Frame):
    ...
    def __init__(self, parent, card_service: CardService, diet_service: DietAppService, 
        ...
        self._update_preview()  # Mostrar previsualización inicial

    def _create_widgets(self):
        """Crea la interfaz del módulo de reportes con previsualización integrada"""
        ...
        # Sección superior: Configuración del reporte
        config_frame = ttk.LabelFrame(main_container, text="Configuración del Reporte", padding="15")
        ...
        self.report_combo = ttk.Combobox(config_frame, values=self.report_types, 
                                         state='readonly', font=('Arial', 10))
        self.report_combo.grid(row=1, column=0, sticky='ew', pady=(0, 15))
        ...
        self.params_frame = ttk.LabelFrame(config_frame, text="Parámetros del Reporte", padding="15")
        self.params_frame.grid(row=2, column=0, sticky='ew', pady=(0, 15))
        self.params_frame.columnconfigure(0, weight=1)
        
        # Inicializar parámetros para el primer reporte
        self._create_card_report_params()
        
        # Selección de formato y botón de generación
        action_frame = ttk.Frame(config_frame, style='Content.TFrame')
        action_frame.grid(row=3, column=0, sticky='ew')
        action_frame.columnconfigure(1, weight=1)
        
        ttk.Label(action_frame, text="Formato:", 
                  font=('Arial', 11, 'bold')).grid(row=0, column=0, sticky='w', padx=(0, 10))
        
        self.format_combo = ttk.Combobox(action_frame, values=self.formats, 
                                         state='readonly', font=('Arial', 10), width=15)
        self.format_combo.grid(row=0, column=1, sticky='w')
        self.format_combo.set(self.formats[0])
        
        self.generate_btn = ttk.Button(action_frame, text="Generar Reporte Completo", 
                                       command=self._generate_report, style='Accent.TButton')
        self.generate_btn.grid(row=0, column=2, sticky='e', padx=(10, 0))
        
        # Sección inferior: Previsualización
        preview_frame = ttk.LabelFrame(main_container, text="Vista Previa del Reporte", padding="10")
        preview_frame.grid(row=1, column=0, sticky='nsew', padx=10, pady=(0, 10))
        preview_frame.columnconfigure(0, weight=1)
        preview_frame.rowconfigure(0, weight=1)
        
        # Widget de texto para previsualización
        ...
        self.preview_text.tag_configure("title", font=("Arial", 12, "bold"), foreground="#2c3e50")
        self.preview_text.tag_configure("header", font=("Arial", 10, "bold"), foreground="#34495e")
        self.preview_text.tag_configure("data", font=("Consolas", 9), foreground="#2c3e50")
        self.preview_text.tag_configure("summary", font=("Arial", 9, "italic"), foreground="#7f8c8d")
        self.preview_text.tag_configure("error", font=("Arial", 10), foreground="#e74c3c")
        self.preview_text.tag_configure("success", font=("Arial", 9, "bold"), foreground="#27ae60")
        self.preview_text.tag_configure("warning", font=("Arial", 9), foreground="#f39c12")
        
        # Scrollbar
        preview_scrollbar = ttk.Scrollbar(text_frame, command=self.preview_text.yview)
    ...
    def _create_card_report_params(self):
        ...
        # Filtro por estado
        ttk.Label(self.params_frame, text="Estado:").grid(row=row, column=0, sticky='w', padx=(0, 10))
        self.status_var = tk.StringVar(value="Todos")
        status_combo = ttk.Combobox(self.params_frame, textvariable=self.status_var, 
                                    values=["Todos", "Activas", "Inactivas"], 
                                    state='readonly', width=15)
        status_combo.grid(row=row, column=1, sticky='w')
        self.status_var.trace('w', lambda *args: self._update_preview())
        row += 1
        
        # Filtro por saldo mínimo
        ttk.Label(self.params_frame, text="Saldo Mínimo:").grid(row=row, column=0, sticky='w', padx=(0, 10))
        self.min_balance_var = tk.DoubleVar(value=0.0)
        min_balance_spin = ttk.Spinbox(self.params_frame, from_=0, to=100000, 
                                       textvariable=self.min_balance_var, width=15,
                                       command=self._update_preview)
        min_balance_spin.grid(row=row, column=1, sticky='w')
        self.min_balance_var.trace('w', lambda *args: self._update_preview())
        row += 1
        
        # Ordenar por
        ttk.Label(self.params_frame, text="Ordenar por:").grid(row=row, column=0, sticky='w', padx=(0, 10))
        self.sort_var = tk.StringVar(value="Número")
        sort_combo = ttk.Combobox(self.params_frame, textvariable=self.sort_var, 
                                  values=["Número", "Saldo", "Fecha de Creación"], 
                                  state='readonly', width=15)
        sort_combo.grid(row=row, column=1, sticky='w')
        self.sort_var.trace('w', lambda *args: self._update_preview())

    def _create_request_user_report_params(self):
        """Crea parámetros para reportes de solicitantes"""
        # Filtro por departamento
        ttk.Label(self.params_frame, text="Departamento:").grid(row=0, column=0, sticky='w', padx=(0, 10))
        ...
        dept_combo = ttk.Combobox(self.params_frame, textvariable=self.dept_filter_var, 
                                  values=dept_names, state='readonly', width=20)
        dept_combo.grid(row=0, column=1, sticky='w')
        self.dept_filter_var.trace('w', lambda *args: self._update_preview())
    ...
    def _create_department_report_params(self):
        ...
        include_check = ttk.Checkbutton(self.params_frame, text="Incluir conteo de solicitantes", 
                                        variable=self.include_count_var,
                                        command=self._update_preview)
        include_check.grid(row=0, column=0, sticky='w', columnspan=2)

    def _update_preview(self, event=None):
        """Actualiza automáticamente la vista previa"""
        try:
            report_type = self.report_combo.get()
            ...
            # Mostrar encabezado
            self.preview_text.insert(tk.END, f"VISTA PREVIA: {report_type}\n", "title")
            self.preview_text.insert(tk.END, f"{'='*50}\n\n", "header")
            
            # Obtener datos según el tipo de reporte
            if "Tarjeta" in report_type:
                data = self._get_card_data(report_type)
                self._show_card_preview(data, report_type)
            elif "Solicitante" in report_type:
                data = self._get_request_user_data(report_type)
                self._show_request_user_preview(data, report_type)
            elif "Departamento" in report_type:
                data = self._get_department_data(report_type)
                self._show_department_preview(data, report_type)
            ...
            self.info_label.config(
                text=f"Vista previa actualizada: {datetime.now().strftime('%H:%M:%S')}"
            )
            
        except Exception as e:
            self.preview_text.config(state=tk.NORMAL)
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, f"Error al generar vista previa:\n{str(e)}", "error")
            self.preview_text.config(state=tk.DISABLED)

    def _show_card_preview(self, data, report_type):
        """Muestra previsualización para reportes de tarjetas"""
        # Mostrar parámetros aplicados
        self.preview_text.insert(tk.END, "Parámetros aplicados:\n", "header")
        self.preview_text.insert(tk.END, f"  • Estado: {self.status_var.get()}\n", "data")
        self.preview_text.insert(tk.END, f"  • Saldo mínimo: ${self.min_balance_var.get():.2f}\n", "data")
        self.preview_text.insert(tk.END, f"  • Ordenar por: {self.sort_var.get()}\n\n", "data")
        
        if not data:
            self.preview_text.insert(tk.END, "No hay datos que coincidan con los filtros aplicados.\n\n", "warning")
            return
        
        # Mostrar resumen
        total_cards = len(data)
        active_cards = sum(1 for card in data if card['Estado'] == 'Activa')
        total_balance = sum(float(card['Saldo'].replace('$', '').replace(',', '')) for card in data)
        
        self.preview_text.insert(tk.END, "Resumen:\n", "header")
        self.preview_text.insert(tk.END, f"  • Total de tarjetas: {total_cards}\n", "data")
        self.preview_text.insert(tk.END, f"  • Tarjetas activas: {active_cards}\n", "data")
        self.preview_text.insert(tk.END, f"  • Tarjetas inactivas: {total_cards - active_cards}\n", "data")
        self.preview_text.insert(tk.END, f"  • Saldo total: ${total_balance:.2f}\n\n", "data")
        
        # Mostrar muestra de datos (máximo 10 filas)
        max_rows = min(10, len(data))
        self.preview_text.insert(tk.END, f"Muestra de datos (primeras {max_rows} filas):\n", "header")
        ...
        for i in range(max_rows):
            ...
            for j, key in enumerate(headers):
                col_widths[j] = max(col_widths[j], len(str(row.get(key, ''))))
        ...
        for h, w in zip(headers, col_widths):
            header_row += f" {h:<{w}} |"
        self.preview_text.insert(tk.END, header_row + '\n', "header")
        self.preview_text.insert(tk.END, separator)
        ...
        for i in range(max_rows):
            ...
            for j, key in enumerate(headers):
                ...
                data_row += f" {value:<{col_widths[j]}} |"
            self.preview_text.insert(tk.END, data_row + '\n', "data")
        ...
        if len(data) > max_rows:
            self.preview_text.insert(tk.END, 
                f"Nota: Mostrando {max_rows} de {len(data)} registros totales.\n"
                f"El reporte completo incluirá todos los registros.\n\n", 
                "summary")

    def _show_request_user_preview(self, data, report_type):
        """Muestra previsualización para reportes de solicitantes"""
        # Mostrar parámetros aplicados
        self.preview_text.insert(tk.END, "Parámetros aplicados:\n", "header")
        self.preview_text.insert(tk.END, f"  • Departamento: {self.dept_filter_var.get()}\n\n", "data")
        ...
        # Contar por departamento si aplica
        if self.dept_filter_var.get() == "Todos":
            dept_counts = {}
            for user in data:
                dept = user.get('Departamento', 'N/A')
                dept_counts[dept] = dept_counts.get(dept, 0) + 1
        
        self.preview_text.insert(tk.END, "Resumen:\n", "header")
        self.preview_text.insert(tk.END, f"  • Total de solicitantes: {total_users}\n", "data")
        
        if self.dept_filter_var.get() == "Todos" and dept_counts:
            self.preview_text.insert(tk.END, "  • Distribución por departamento:\n", "data")
            for dept, count in sorted(dept_counts.items(), key=lambda x: x[1], reverse=True)[:5]:
                percentage = (count / total_users) * 100
                self.preview_text.insert(tk.END, f"      - {dept}: {count} ({percentage:.1f}%)\n", "data")
            if len(dept_counts) > 5:
                self.preview_text.insert(tk.END, f"      ... y {len(dept_counts) - 5} departamentos más\n", "data")
        
        self.preview_text.insert(tk.END, "\n")
        
        # Mostrar muestra de datos
        max_rows = min(8, len(data))
        self.preview_text.insert(tk.END, f"Muestra de datos (primeras {max_rows} filas):\n", "header")
        
        for i in range(max_rows):
            user = data[i]
            self.preview_text.insert(tk.END, 
                f"{i+1}. {user.get('Nombre', 'N/A')}\n"
    ...
    def _show_department_preview(self, data, report_type):
        """Muestra previsualización para reportes de departamentos"""
        # Mostrar parámetros aplicados
        self.preview_text.insert(tk.END, "Parámetros aplicados:\n", "header")
        self.preview_text.insert(tk.END, 
            f"  • Incluir conteo de solicitantes: {'Sí' if self.include_count_var.get() else 'No'}\n\n", 
            "data")
        
        if not data:
            self.preview_text.insert(tk.END, "No hay datos disponibles.\n\n", "warning")
            return
        
        # Mostrar resumen
        total_depts = len(data)
        total_users = sum(d.get('Solicitantes', 0) for d in data) if self.include_count_var.get() else 0
        
        self.preview_text.insert(tk.END, "Resumen:\n", "header")
        self.preview_text.insert(tk.END, f"  • Total de departamentos: {total_depts}\n", "data")
        if self.include_count_var.get():
            self.preview_text.insert(tk.END, f"  • Total de solicitantes: {total_users}\n", "data")
            avg_users = total_users / total_depts if total_depts > 0 else 0
            self.preview_text.insert(tk.END, f"  • Promedio por departamento: {avg_users:.1f}\n", "data")
        self.preview_text.insert(tk.END, "\n")
        
        # Mostrar datos
        self.preview_text.insert(tk.END, "Lista de departamentos:\n", "header")
        
        for i, dept in enumerate(data[:10]):  # Mostrar máximo 10
            dept_info = f"{i+1}. {dept.get('Nombre', 'N/A')}"
            if self.include_count_var.get():
                dept_info += f" - {dept.get('Solicitantes', 0)} solicitantes"
            self.preview_text.insert(tk.END, dept_info + "\n", "data")
        
        if len(data) > 10:
            self.preview_text.insert(tk.END, f"\n... y {len(data) - 10} departamentos más\n", "summary")

    def _generate_report(self):
        """Genera el reporte completo según los parámetros seleccionados"""
        report_type = self.report_combo.get()
    ...
    def _get_card_data(self, report_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudieron obtener las tarjetas: {str(e)}")
            return []
        ...
        for card in cards:
            ...
            if balance < self.min_balance_var.get():
                continue
            ...
            card_data = {
                ...
                'Creada': getattr(card, 'created_at', 'N/A').strftime('%d/%m/%Y') 
                         if hasattr(card, 'created_at') and card.created_at else 'N/A',
                'Última Actualización': getattr(card, 'updated_at', 'N/A').strftime('%d/%m/%Y %H:%M') 
                                      if hasattr(card, 'updated_at') and card.updated_at else 'N/A'
            }
        ...
        elif sort_key == "Fecha de Creación":
            data.sort(key=lambda x: x['Creada'], reverse=True)
        
        # Aplicar filtro de estado
        if self.status_var.get() == "Activas":
            data = [d for d in data if d['Estado'] == 'Activa']
        elif self.status_var.get() == "Inactivas":
            data = [d for d in data if d['Estado'] == 'Inactiva']
    ...
    def _get_request_user_data(self, report_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            messagebox.showerror("Error", f"No se pudieron obtener los solicitantes: {str(e)}")
            return []
        ...
        for user in request_users:
            ...
            user_data = {
                ...
                'Fecha Registro': getattr(user, 'created_at', 'N/A').strftime('%d/%m/%Y') 
                               if hasattr(user, 'created_at') and user.created_at else 'N/A'
            }
    ...
    def _generate_pdf(self, data: Any, report_type: str, default_name: str):
        ...
        try:
            ...
            msg = f"Reporte PDF generado exitosamente:\n{file_path}"
            ...
            if messagebox.askyesno("Éxito", f"{msg}\n\n¿Desea abrir el reporte generado?"):
                try:
                    ...
                    messagebox.showinfo("Abrir Reporte", 
                        "No se pudo abrir el archivo automáticamente. "
                        "Por favor, ábralo manualmente desde la ubicación guardada.")
        ...
        try:
            if isinstance(data, list) and data:
                ...
                with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
                    ...
                    # Acceder al workbook y worksheet para formato
                    ...
                    for column in df:
                        ...
                        worksheet.column_dimensions[chr(64 + col_idx)].width = column_length
                    
                    # Formato para encabezados
                    for cell in worksheet[1]:
                        cell.font = pd.ExcelWriter._get_default_style()['font'].copy(bold=True)
                        cell.fill = pd.ExcelWriter._get_default_style()['fill'].copy(
                            patternType='solid', 
                            fgColor='366092'
                        )
                        cell.font = pd.ExcelWriter._get_default_style()['font'].copy(color='FFFFFF')
                
            elif isinstance(data, dict):
                # Datos de estadísticas
                # Filtrar clave 'error' si existe
                filtered_data = {k: v for k, v in data.items() if k != 'error'}
            ...
            # Mostrar mensaje de éxito
            msg = f"Reporte Excel generado exitosamente:\n{file_path}"
            ...
            messagebox.showerror("Error", 
                "openpyxl no está instalado. Instálelo con: pip install openpyxl")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo generar el Excel: {str(e)}")

    def refresh_data(self):
        """Actualiza los datos del módulo"""
        # Actualizar previsualización con datos frescos
        self._update_preview()
...
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Prueba de ReportModule con Previsualización Integrada")
    root.geometry("1000x800")
    
    # Mock de servicios para pruebas
    class MockCard:
        def __init__(self, number, active=True, balance=100.0):
    ...
    class MockUser:
        def __init__(self, name, ci, dept_name):
            self.fullname = name
            self.ci = ci
            self.department = type('Dept', (), {'name': dept_name, 'id': 1})()
    ...
    class MockCardService:
        def get_all_cards(self):
            return [
                MockCard("100001", True, 150.0),
                MockCard("100002", False, 0.0),
                MockCard("100003", True, 75.5),
                MockCard("100004", True, 200.0),
                MockCard("100005", False, 50.0),
            ]
    
    class MockRequestService:
        def get_all_request_users(self):
            return [
                MockUser("Juan Pérez", "12345678", "Ventas"),
                MockUser("María García", "87654321", "Ventas"),
                MockUser("Carlos López", "11223344", "IT"),
                MockUser("Ana Martínez", "44332211", "RH"),
                MockUser("Pedro Rodríguez", "55667788", "IT"),
            ]
    
    class MockDeptService:
    ...
    report_module = ReportModule(
        root,
        MockCardService(),
        None,  # diet_service no se usa en pruebas básicas
        MockRequestService(),
        MockDeptService()
    )
    
    report_module.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    root.mainloop()